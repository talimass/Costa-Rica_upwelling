...
fire.data <- read.csv("/Users/talimass/Documents/Documents - MacBook Pro/GitHub/Costa-Rica_upwelling/Data/Samara_anotated _xls.csv", stringsAsFactors = TRUE)
...
# Create the plot
TauAv1_plot <- ggplot(data = TauAv1_no_outlier, aes(x = TauAv1, y = Site_depth, fill = Species)) +  # Use 'pmax' here
  geom_boxplot() +
  stat_boxplot(geom = "errorbar", width = 0.75) +  # Removed position="dodge"
  theme_classic() +
  ...
  theme(legend.position = "none") +
  facet_wrap(~ Species, scales = "free_y") +  # Each species gets a separate panel
  scale_y_discrete(limits = rev(levels(factor(TauAv1_no_outlier$Site_depth))))  # Reverse order for readability
...
TauAv1 <- ggplot(data = TauAv1_no_outlier, aes(x = TauAv1, y = Site_depth, fill = Species)) +
  geom_boxplot(alpha = 0.6, outlier.shape = NA) +  # Boxplot with slight transparency, hides outliers
  geom_jitter(color = "black", width = 0.1, size = 1, alpha = 0.6) +  # Black points, smaller size
  stat_boxplot(geom = "errorbar", width = 0.75) +  # Error bars for boxplots
  theme_classic() +
  labs(title = "Qa reoxidation kinetics in darkt", 
       x = "TauAv1", 
       y = "Site - Depth") +  # Y-axis now represents Site-Depth
  
  theme(legend.position = "none") +
  facet_wrap(~ Species, scales = "free_y", ncol = 2) +  # Facet by Species with 2 columns
  scale_y_discrete(limits = rev(levels(factor(TauAv1_no_outlier$Site_depth)))) 
...
# Save the plot
ggsave(filename = file_name, plot = TauAv1, width = 8, height = 6, dpi = 300)
...
```{r}
test.vars=c("depth", "Site", "fv_fm", "Sigma", "p", "Pmax.e.s")
...
Leven_test=lapply(fire.data.to.test[,3:6], leveneTest, group = fire.data.to.test$depth)
Leven_test
```
...
```{r}
#One-way ANOVA test
aov_fire.data <- apply(fire.data.to.test[,3:6], 2, function(x) aov(x ~ depth, data = fire.data.to.test))
...
```{r}
tukey_res <- sapply(aov_fire.data , function(x) TukeyHSD(x, "depth", ordered = TRUE))
...
##Kruskal-Walis test
```{r}
KW_fire.data <- apply(fire.data.to.test[,3:6], 2, function(x) kruskal.test(x ~ depth, data = fire.data.to.test))
...
```{r}
Dunn_fire.data <- apply(fire.data.to.test[,3:6], 2, function(x) dunnTest(x ~ depth, data = fire.data.to.test, table=TRUE))
